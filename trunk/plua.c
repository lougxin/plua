...
zend_module_entry plua_module_entry = {
	...
	PHP_MINIT(plua),
	PHP_MSHUTDOWN(plua),
	PHP_RINIT(plua),		/* Replace with NULL if there's nothing to do at request start */
	PHP_RSHUTDOWN(plua),	/* Replace with NULL if there's nothing to do at request end */
	PHP_MINFO(plua),
#if ZEND_MODULE_API_NO >= 20010901
...
static zval * plua_call_lua_function(lua_State *L, zval *func, zval *args, int use_self TSRMLS_DC) {
	...
	if (IS_ARRAY == Z_TYPE_P(func)) {
		...
		if (LUA_TFUNCTION != lua_type(L, lua_gettop(L))) {
			lua_pop(L, -1);
			plua_warn("invalid lua function resource(%d) of type (%s)", (long)ref_id, PHP_PLUA_FUNCTION_RESOURCE_NAME);
			return NULL;
		}   
	}
